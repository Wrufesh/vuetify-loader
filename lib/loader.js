...
module.exports = async function (content, sourceMap) {
  ...
  if (!options.disableDefaultMatchers){
  ...
  if (!this.resourceQuery) {
    ...
    if (component.template) {
      ...
      if(component.script && component.script.content){
        let scriptIcons = component.script.content.match(/(?<=\$getIcon\(("|')).*(?=("|')\))/g)
        if (scriptIcons && scriptIcons.length > 0){
          scriptIcons.forEach(item => icons.add(item))
        }  
      }
